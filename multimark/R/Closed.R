...
simdataClosedSCR <- function(N=100,ntraps=16,noccas=5,pbeta=log(0.4),tau=0,sigma2_zp=0.1,delta_1=0.4,delta_2=0.4,alpha=0.5,data.type="never",detection="half-normal",spatialInputs=NULL){
  ...
  if(is.null(spatialInputs)){
    ...
    if(N!=nrow(spatialInputs$centers)){
      warning("'N' not equal to the number of rows of spatialInputs$centers; the latter will be used.")
      N<-nrow(spatialInputs$centers)
    }
  }
...
pstarintegrandSCR<-function(beta,sigma2,DM,spatialInputs,dexp){
  
  XB <- DM %*% beta
  dist2 <- spatialInputs$dist2
  detProb<-invcloglog(matrix(XB,nrow=dim(dist2)[1],ncol=dim(dist2)[2]*nrow(XB),byrow=TRUE)-1/(2*sigma2)*dist2[,rep(1:dim(dist2)[2],each=nrow(XB))]^dexp)
  pdot<-1.-apply(1.-detProb,1,function(x) max(prod(x),1.e-10))
...
#'   
#'  \code{centers} is a matrix containing the true (latent) coordinates of the activity centers for each individual in the population, where each row corresponds to an individual, the first column the x-coordinate, and the second column the y-coordinate.
#'
...
#' summary(bobcat.dot$mcmc)
#' plot(bobcat.dot$mcmc)}
multimarkClosedSCR<-function(Enc.Mat,data.type="never",covs=data.frame(),mms=NULL,spatialInputs=NULL,mod.p=~1,mod.delta=~type,detection="half-normal",parms=c("pbeta","delta","N","sigma2_zp"),nchains=1,iter=12000,adapt=1000,bin=50,thin=1,burnin=2000,taccept=0.44,tuneadjust=0.95,proppbeta=0.1,propzp=1,propsigmap=1,maxnumbasis=1,a0delta=1,a0alpha=1,b0alpha=1,a=25,mu0=0,sigma2_mu0=1.75,a0psi=1,b0psi=1,initial.values=NULL,known=integer(),printlog=FALSE,...){
  ...
  inits<-get_initsSCR(mms,nchains,initial.values,M,data.type,a0alpha,b0alpha,a0delta,a0psi,b0psi,DM,availSpatialInputs,ifelse(detection=="half-normal",2,1))
